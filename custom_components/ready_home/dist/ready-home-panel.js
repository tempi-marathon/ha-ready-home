 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */const K=globalThis,tt=s=>s,B=K.trustedTypes,et=B?B.createPolicy("lit-html",{createHTML:s=>s}):void 0,ct="$lit$",y=`lit$${Math.random().toFixed(9).slice(2)}$`,dt="?"+y,At=`<${dt}>`,w=document,T=()=>w.createComment(""),M=s=>s===null||typeof s!="object"&&typeof s!="function",Z=Array.isArray,Ct=s=>Z(s)||typeof s?.[Symbol.iterator]=="function",F=`[ 	
\f\r]`,S=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,it=/-->/g,rt=/>/g,v=RegExp(`>|${F}(?:([^\\s"'>=/]+)(${F}*=${F}*(?:[^ 	
